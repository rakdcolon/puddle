...
try {
  if (command === 'list') {
    ...
    if (command === 'show') console.log(JSON.stringify({id,puzzle,review,source},null,2));
      ...
      const target = join(ROOT,'puzzles',`${String(puzzle.issue_no).padStart(3,'0')}-agent.json`);
      ...
      if (existsSync(target)) {
        ...
        if (archive().some(p => p.issue_no === puzzle.issue_no || p.date_active === puzzle.date_active)) throw new Error('Issue/date now conflicts with archive');
        ...
        const provenance = join(provenanceDir,`${puzzle.issue_no}.json`);
